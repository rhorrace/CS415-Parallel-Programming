//Horrace, Robert
/*
	This program is the modified version
	of qsort.c. This version will use openMP.
*/
...
void quicksort(int *array, int low, int high, int tid) {
    		...
    		bubblesort(array, low, high,tid);
				threads[tid] += high-low;
				return;
			}
  		int middle = partition(array, low, high, tid);
			#pragma omp parallel sections shared(threads,array,low,middle,high) private(tid) num_threads(numThreads)
			{
				#pragma omp section
				{
